...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
	...
	private static JMenuBar createMenuBar() {
		...
		JMenu menu1 = new JMenu("Robot");
		JMenu menu3 = new JMenu("Game Map");

		menuBar.add(menu);
		menuBar.add(menu1);
		menuBar.add(menu3);

		JMenuItem menuItem33 = new JMenuItem("Choose Game Map");
		JMenuItem menuItem1 = new JMenuItem("Save...");
		JMenuItem menuItem2 = new JMenuItem("load from...");

		JMenuItem menuItem11 = new JMenuItem("Place your robot");
	...
	private static MyGameGUI my_game ;
	...
	public static void setMyGui( MyGameGUI m) {
		my_game=m;
		//		gui.algo.init(g.g);
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		case "Place your robot":
			if(game_mode.equals("Manual Player")) {
			try {
				String[] arr = new String[gui.g.nodeSize()];
				
			for (int j = 0; j < arr.length; j++) {
				arr[j] =gui.g.getNode(j).getKey() + "";
			}
				for (int i = 0; i < my_game.game_robots.size(); i++) 
				{
					Object Location = JOptionPane.showInputDialog(null, "Place robot number: "+(i+1)+ "\nOn node number: " , "Message",
							JOptionPane.INFORMATION_MESSAGE, null, arr, arr[0]);
					String loc = Location.toString();
				
					 node_data new_node = gui.g.getNode(Integer.parseInt((String )Location));
					my_game.getGame_serv().addRobot(Integer.parseInt(loc));
					 Point3D robot_location = new_node.getLocation();
					my_game.game_robots.get(i).setLocation(robot_location);
					my_game.game_robots.get(i).setSrc(new_node.getKey());
					my_game.drawRobots();
					my_game.setFlag(true);
				}
				if(my_game.isFlag()) {
					String _optins[]= {"Yes","No"};
					Object orderToStart = JOptionPane.showInputDialog(null, "Do you want to start the game??", "Game Mode",
							JOptionPane.INFORMATION_MESSAGE, null, _optins,_optins[0] );
					if((String) orderToStart == "Yes") {
						//turn the game on
					my_game.start_manual_game( my_game.getGame_serv() , my_game ) ;
					}
				}
			}
			catch(Exception ex) {
				 ...
				 JOptionPane.showMessageDialog(null, "Automatic Player: you can't place robots", "Wrong Mode",JOptionPane.ERROR_MESSAGE);
			}
			break;
		case "Choose Game Map":
			try {
				
				Object selectedMap = JOptionPane.showInputDialog(null, "Choose from 0 to 23 the \n Game Map you want to play in ", "Message",
						JOptionPane.INFORMATION_MESSAGE);
				int Map = Integer.parseInt(selectedMap.toString());
				...
				//	if(Map < 0 || Map > 23) { JOptionPane.showMessageDialog(null, "The Map doesn't exist","Messege",0);}
					...
					my_game.Automatic_Player(id ,Map);
				}
				else {
				my_game.setMap(my_game, Map);
				}
						
			}
			catch (Exception e2) {
				JOptionPane.showMessageDialog(null, "You have to insert a number","Messege",0);
			}
